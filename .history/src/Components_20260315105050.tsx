  ...
  return (
    <div className="fixed inset-0 z-[400] bg-[var(--bg-main)] flex flex-col pt-safe">
      ...
      <div className="flex items-center justify-between px-4 h-16 bg-[var(--bg-main)]/80 backdrop-blur-xl sticky top-0 shrink-0 z-20">
        <button 
          onClick={onClose} 
          className="w-[80px] text-left text-[var(--primary)] text-[17px] font-medium active:opacity-50 transition-opacity"
        >
          {t.cancel}
        </button>
        <h2 className="flex-1 text-center text-[17px] font-semibold text-white tracking-tight">
      ...
      {/* 2. HIG Segmented Control (Вместо старых табов) */}
        ...
        <div className="flex bg-[#1C1C1E] p-1 rounded-lg">
          <button 
            ...
            className={`flex-1 py-1.5 text-[13px] font-semibold rounded-md transition-all ${
              activeTab === 'overdue' ? 'bg-[#636366] text-white shadow-sm' : 'text-white/50 hover:text-white/80'
            }`}
          >
          ...
          <button 
            ...
            className={`flex-1 py-1.5 text-[13px] font-semibold rounded-md transition-all ${
              activeTab === 'upcoming' ? 'bg-[#636366] text-white shadow-sm' : 'text-white/50 hover:text-white/80'
            }`}
          >
      ...
      {/* 3. СПИСОК ЗАДАЧ */}
      <div className="flex-1 overflow-y-auto px-4 pt-4 pb-safe-24 space-y-3 custom-scrollbar">
          ...
          displayedTasks.map((task) => {
            ...
            return (
              <div 
                ...
                className="bg-[var(--bg-surface)] rounded-[16px] p-4 flex flex-col shadow-sm cursor-pointer transition-all active:scale-[0.98] active:bg-[var(--bg-surface-elevated)] border border-white/5"
              >
                <div className="flex items-start gap-3">
                  
                  {/* Изящный круглый чекбокс вместо огромной кнопки "Готово" */}
                  <button 
                    onClick={(e) => { e.stopPropagation(); handleMarkDone(task); }}
                    className="mt-0.5 shrink-0 w-6 h-6 rounded-full border-[1.5px] border-white/30 hover:border-[var(--success)] active:bg-[var(--success)]/20 transition-colors flex items-center justify-center text-transparent hover:text-[var(--success)]"
                  >
                  ...
                  <div className="min-w-0 flex-1 pr-2">
                    <h3 className="font-semibold text-white text-[17px] tracking-tight truncate leading-tight">
                      {task.title}
                    </h3>
                    <p className={`text-[13px] mt-1 font-medium ${isOverdue ? 'text-[var(--danger)]' : 'text-white/40'}`}>
                      {task.date} • {task.time}
                    </p>
                  </div>
                  
                  {/* Аккуратная иконка календаря для переноса */}
                  <button 
                    onClick={(e) => { e.stopPropagation(); onReschedule(task); }} 
                    className="p-2 -mt-2 -mr-2 text-[var(--info)] rounded-full active:bg-white/10 transition-colors"
                  >
                    <Calendar size={18} />
                  </button>
                </div>
      ...
      {lastDoneTask && (
        <div className="absolute bottom-10 left-4 right-4 z-[9999] bg-[#2C2C2E] border border-white/10 rounded-[16px] p-4 flex items-center justify-between shadow-2xl animate-in slide-in-from-bottom-5 fade-in duration-300">
          ...
          <button 
            onClick={handleUndo} 
            className="text-[var(--info)] font-bold text-[16px] active:opacity-50 transition-opacity px-4 py-2 bg-[var(--info)]/10 rounded-xl"
          >
            Undo
          </button>
        </div>
