...
import { 
  ...
  Copy, Download, Upload, Plus, ArrowUp, ZapOff, ChevronLeft
} from 'lucide-react';
...
export const InputRow = ({ label, children }: { label: string; children: React.ReactNode }) => (
  <div className="flex items-center justify-between px-5 py-4 border-b border-white/5 last:border-0 min-h-[60px]">
    <span className="text-[16px] text-white/60 font-medium w-28 shrink-0">{label}</span>
...
export const ReviewScreen = ({ 
  ...
  return (
    <div className="fixed inset-0 bg-[var(--bg-main)] z-[150] flex flex-col pt-safe animate-in slide-in-from-bottom-4 duration-300">
      
      {/* HEADER (Исправлены цвета Cancel и Check) */}
      <div className="flex items-center justify-between px-4 h-16 bg-[var(--bg-main)]/80 backdrop-blur-xl sticky top-0 shrink-0 z-30 border-b border-white/5">
        <button onClick={onCancel} className="w-[80px] text-left text-[var(--primary)] text-[17px] active:opacity-50 transition-opacity">
          {t.cancel || 'Cancel'}
        </button>
        <h2 className="flex-1 text-center text-[17px] font-semibold text-white tracking-tight">
        ...
        <div className="w-[80px] flex justify-end">
          <Check className="w-6 h-6 text-[var(--primary)] opacity-80" />
        </div>
      </div>

      {/* CONTENT (Увеличен paddingBottom, чтобы кнопка не перекрывала контент) */}
      <div className="flex-1 overflow-y-auto px-5 pt-4 custom-scrollbar transition-all duration-300" style={{ paddingBottom: `calc(150px + ${kbOffset}px)` }}>
        
        {/* TITLE (Сделали крупнее и массивнее, настоящий iOS Large Title) */}
        <div className="pt-2 pb-6">
          <textarea 
            value={parsedEvent.title} 
            onFocus={handleInputFocus} 
            onChange={e => setParsedEvent({...parsedEvent, title: e.target.value})} 
            className="w-full bg-transparent text-white text-[32px] leading-tight font-bold outline-none resize-none placeholder:text-white/20" 
            rows={2} 
          />
        </div>

        {/* BLOCK 1: Date & Time */}
        <div className="bg-[var(--bg-surface)] rounded-[20px] border border-white/5 overflow-hidden mb-5 shadow-lg">
          <InputRow label={t.date}>
            <input type="date" value={parsedEvent.date} onFocus={handleInputFocus} onChange={e => setParsedEvent({...parsedEvent, date: e.target.value})} className="bg-transparent text-right outline-none text-white text-[17px] font-medium" />
          </InputRow>
          <InputRow label={t.time}>
            <input type="time" value={parsedEvent.time} onFocus={handleInputFocus} onChange={e => setParsedEvent({...parsedEvent, time: e.target.value})} className="bg-transparent text-right outline-none text-white text-[17px] font-medium" />
          </InputRow>
          <InputRow label={t.duration}>
            <select value={parsedEvent.duration} onFocus={handleInputFocus} onChange={e => setParsedEvent({...parsedEvent, duration: e.target.value})} className="bg-transparent text-white text-[17px] font-medium outline-none appearance-none cursor-pointer w-full text-right text-align-last-right">
        ...
        {/* BLOCK 2: Location & Guests */}
        <div className="bg-[var(--bg-surface)] rounded-[20px] border border-white/5 overflow-hidden mb-5 shadow-lg relative">
          <InputRow label={t.location}>
            <input type="text" value={parsedEvent.location} onFocus={handleInputFocus} onChange={e => onLocationChange(e.target.value)} className="bg-transparent text-right outline-none text-white w-full min-w-0 placeholder:text-white/20 text-[17px]" placeholder={t.address || "Address"} />
            {parsedEvent.location && <button onClick={() => onLocationChange('')} className="ml-2 p-1 text-white/40 hover:text-white/80 active:scale-95 transition-all"><X size={18} /></button>}
          ...
          {showLocationDropdown && (
            <div className="absolute z-50 left-0 right-0 top-[60px] bg-[var(--bg-surface-elevated)] border-t border-white/10 shadow-2xl max-h-48 overflow-y-auto custom-scrollbar">
              {locationSuggestions.map((loc, i) => (
                <button key={i} onClick={() => onSelectLocation(loc)} className="w-full text-left px-5 py-3.5 text-[15px] text-white hover:bg-white/10 border-b border-white/5 last:border-0 truncate transition-colors"><MapPin size={16} className="inline mr-3 text-[var(--primary)]" /> {loc}</button>
              ))}
            </div>
          )}
          
          <InputRow label={t.guestsLabel || "Guests"}>
            <input type="text" value={parsedEvent.guests} onFocus={handleInputFocus} onChange={e => setParsedEvent({...parsedEvent, guests: e.target.value})} className="bg-transparent text-right outline-none text-white w-full min-w-0 placeholder:text-white/20 text-[16px]" placeholder="email1, email2" />
            {parsedEvent.guests && <button onClick={() => setParsedEvent({...parsedEvent, guests: ''})} className="ml-2 p-1 text-white/40 hover:text-white/80 active:scale-95 transition-all"><X size={18} /></button>}
          </InputRow>
        </div>
        
        {/* BLOCK 3: Task Mode */}
        <div className="bg-[var(--bg-surface)] rounded-[20px] border border-white/5 overflow-hidden mb-5 shadow-lg">
          <InputRow label={t.taskMode || "Task Control"}>
            <Toggle checked={!!parsedEvent.isTask} onChange={(v) => setParsedEvent({...parsedEvent, isTask: v})} />
          </InputRow>
        </div>

        {/* BLOCK 4: Description */}
        <div className="bg-[var(--bg-surface)] rounded-[20px] border border-white/5 overflow-hidden mb-4 p-5 shadow-lg">
          <label className="text-[12px] text-white/40 mb-3 font-bold uppercase tracking-widest block">{t.descLabel || "Description"}</label>
          <textarea 
            value={parsedEvent.description} 
            onFocus={handleInputFocus} 
            onChange={e => setParsedEvent({...parsedEvent, description: e.target.value})} 
            className="w-full bg-transparent text-[16px] leading-relaxed text-white/90 outline-none resize-none placeholder:text-white/20" 
            rows={8} 
          />
        </div>
      </div>

      {/* BOTTOM ACTION BUTTON (Исправлен градиент и тень) */}
      <div className="fixed bottom-0 left-0 right-0 pt-10 pb-safe-16 px-5 z-20 pointer-events-none" 
           style={{ background: 'linear-gradient(to top, rgba(0,0,0,1) 0%, rgba(0,0,0,0.9) 40%, rgba(0,0,0,0) 100%)' }}>
        <button 
          onClick={onSave} 
          className="w-full h-[56px] bg-[var(--primary)] text-white font-bold text-[17px] rounded-2xl shadow-[0_8px_30px_rgba(var(--primary-rgb),0.3)] active:scale-95 transition-all pointer-events-auto"
        >
          {t.saveToCalendar || "Save to Calendar"}
        </button>
      </div>
    </div>
  );
