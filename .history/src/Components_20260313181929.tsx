...
const Toggle = ({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) => (
  <button onClick={() => onChange(!checked)} className={`w-[51px] h-[31px] rounded-full transition-colors relative focus:outline-none ${checked ? 'bg-[#5E5CE6]' : 'bg-[#39393D]'}`}>
  ...
  return (
    <div className="fixed inset-0 z-[250] bg-black/90 backdrop-blur-xl flex items-center justify-center p-4">
      <div className="bg-[#1C1C1E] border border-white/10 rounded-2xl w-full max-w-sm p-6 flex flex-col shadow-2xl">
         <div className="flex items-center gap-3 mb-4 text-[#5E5CE6]">
           <History size={24} />
                ...
                <div className="flex justify-between items-start gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div 
                      ...
                      className={`flex items-start gap-1.5 text-white/30 ${!onSelect ? 'cursor-pointer hover:text-[#5E5CE6]' : ''}`} 
                    >
