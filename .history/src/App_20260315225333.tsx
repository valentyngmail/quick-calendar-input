          ...
          {phase === 'idle' && (
            <div className={`relative transition-all duration-500 w-full ${isFocused ? 'scale-[1.02]' : 'scale-100'}`}>
              <textarea
                ...
                rows={1}
                className="w-full bg-transparent text-center text-[28px] leading-tight font-semibold outline-none resize-none py-10 px-6 relative z-10 custom-scrollbar placeholder-animated"
              />
            </div>
          )}
      ...
      {phase === 'idle' && (
        <div className="absolute left-6 right-6 flex justify-center z-50 transition-all duration-300" style={{ bottom: 'calc(env(safe-area-inset-bottom) + 20px)' }}>
          <div className="bg-[#1C1C1E]/80 backdrop-blur-2xl border border-white/10 rounded-full p-1.5 flex items-center w-full max-w-sm shadow-2xl">
            ...
            {/* Массивная кнопка действия с ArrowUp (Динамическая) */}
            <button 
              onClick={handleProcessText} 
              disabled={!textInput.trim()}
              className={`flex-1 h-[46px] ml-2 rounded-full font-semibold text-[17px] flex items-center justify-center gap-2 transition-all duration-300 ${
                textInput.trim() 
                  ? 'bg-[var(--primary)] text-white shadow-[0_0_20px_rgba(var(--primary-rgb),0.3)] active:scale-[0.97]' 
