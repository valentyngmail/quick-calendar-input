...
import { 
  ...
  BookOpen, Search, MapPin, Edit2, Trash2, Calendar, Clock, ArrowRight, Copy, Download, Upload, ClipboardPaste
} from 'lucide-react';
...
export const SettingsModal = ({ 
  ...
  // --- ФУНКЦИЯ ВСТАВКИ ИЗ БУФЕРА (ДЛЯ МИГРАЦИИ) ---
  const pasteFromClipboard = async () => {
    try {
      ...
      if (Array.isArray(json) && json.length > 0) {
        localStorage.setItem(FAV_PLACES_KEY, JSON.stringify(json));
        toast.success(`Imported ${json.length} places!`);
        setTimeout(() => window.location.reload(), 1500);
      } else {
        toast.error('Invalid database format in clipboard');
      }
    } catch (err) {
      toast.error('Could not read clipboard. Please allow access.');
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black flex flex-col overflow-hidden" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-32">
        ...
        <div className="bg-[#1C1C1E] rounded-xl px-4 overflow-hidden">
          <div className="py-3 border-b border-white/10">
             <div className="flex items-center gap-3 mb-1"><div className="w-7 h-7 rounded-md bg-[#8E8E93] flex items-center justify-center text-white"><LinkIcon size={16} /></div><span className="text-[17px]">Decode Webhook</span></div>
          ...
          <div className="py-3 border-b border-white/10">
             <div className="flex items-center gap-3 mb-1"><div className="w-7 h-7 rounded-md bg-[#8E8E93] flex items-center justify-center text-white"><LinkIcon size={16} /></div><span className="text-[17px]">Save Webhook</span></div>
          ...
          <button onClick={() => { onClose(); onOpenSyncModal(); }} className="w-full flex items-center py-3 text-[#34C759] gap-3">
             ...
             <span className="text-[17px] font-medium">{t.syncHistory || 'Sync 3-Year History'}</span>
          </button>
        </div>

        {/* НОВЫЙ БЛОК БЭКАПОВ */}
        <p className="text-[13px] text-white/40 uppercase mt-8 mb-2 ml-4 tracking-tight">Database Backup</p>
        <div className="bg-[#1C1C1E] rounded-xl px-4 overflow-hidden flex flex-col">
          <button onClick={pasteFromClipboard} className="flex items-center gap-3 py-3 border-b border-white/10 text-[#34C759] active:opacity-50 transition-opacity">
            <div className="w-7 h-7 rounded-md bg-[#34C759] flex items-center justify-center text-black"><ClipboardPaste size={16} /></div>
            <span className="text-[17px] font-medium">Import from Clipboard</span>
          </button>
          ...
          <button onClick={exportBackup} className="flex items-center gap-3 py-3 border-b border-white/10 text-[#0A84FF] active:opacity-50 transition-opacity">
            ...
            <span className="text-[17px] font-medium">Export to File</span>
          </button>
          ...
          <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-3 py-3 text-[#FF9500] active:opacity-50 transition-opacity">
            ...
            <span className="text-[17px] font-medium">Restore from File</span>
          </button>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black via-black/90 to-transparent pb-[calc(env(safe-area-inset-bottom)+16px)]">
        <button onClick={handleSave} className="w-full h-14 bg-[#34C759] text-black font-bold rounded-2xl shadow-xl active:scale-95 transition-transform">{t.saveSettings || 'Save Configuration'}</button>
      </div>
    </div>
  );
};
