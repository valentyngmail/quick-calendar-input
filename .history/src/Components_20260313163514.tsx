    ...
    <div className="flex-1 overflow-y-auto px-4 pb-32">
      ...
      <div className="bg-[#1C1C1E] rounded-xl overflow-hidden mb-6">
        <InputRow label={t.date}>
          <input type="date" value={parsedEvent.date} onChange={e => setParsedEvent({...parsedEvent, date: e.target.value})} className="bg-transparent text-right outline-none text-[#34C759] font-bold" />
        </InputRow>
        <InputRow label={t.time}>
          <input type="time" value={parsedEvent.time} onChange={e => setParsedEvent({...parsedEvent, time: e.target.value})} className="bg-transparent text-right outline-none text-[#34C759] font-bold" />
        </InputRow>
        <InputRow label={t.duration}>
          {/* Используем textAlignLast для выравнивания текста внутри выпадающего списка на iOS */}
          <select 
      ...
      <div className="bg-[#1C1C1E] rounded-xl overflow-hidden mb-6">
        <InputRow label={t.taskMode || "Контроль"}>
          <Toggle 
            checked={!!parsedEvent.isTask} 
            onChange={(v) => setParsedEvent({...parsedEvent, isTask: v})} 
          />
        </InputRow>
      </div>
...
export const TasksListModal = ({ open, onClose, tasks, onMarkDone, onReschedule, t }: TasksListModalProps) => {
  // Состояние, чтобы помнить, какая карточка сейчас раскрыта
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-black flex flex-col" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
      <div className="flex items-center justify-between px-4 h-11 border-b border-white/10 bg-black/80 backdrop-blur-xl sticky top-0 shrink-0">
        ...
        <h2 className="text-[17px] font-semibold">{t.tasksTitle || "Мои Хвосты"}</h2>
        <div className="w-[60px]" />
      </div>
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {tasks.length === 0 ? (
          ...
          tasks.map((task) => (
            <div 
              key={task.id} 
              // Нажатие на всю плашку открывает/закрывает описание
              onClick={() => setExpandedId(expandedId === task.id ? null : task.id)}
              className="bg-[#1C1C1E] border-l-4 border-l-red-500 rounded-2xl p-4 flex flex-col shadow-lg cursor-pointer transition-colors hover:bg-white/5"
            >
              <div className="flex justify-between items-start">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {/* Иконка перепланирования (Reschedule) */}
                  <button 
              ...
              {/* Выпадающее описание (раскрывается только если expandedId совпадает) */}
