...
export const TasksListModal = ({ open, onClose, tasks, onMarkDone, onReschedule, t }: TasksListModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-black flex flex-col pt-safe">
      ...
      {/* ПЕРЕДЕЛАННЫЙ ПЕРЕКЛЮЧАТЕЛЬ (Native Segmented Control Style) */}
      <div className="px-4 py-2 shrink-0">
        <div className="flex items-center p-0.5 bg-[#1C1C1E]/60 border border-white/5 rounded-full overflow-hidden w-full backdrop-blur-sm shadow-inner-dark">
          ...
          >
            ...
            <span className={`px-2.5 py-0.5 rounded-full text-[12px] font-bold ${
              activeTab === 'overdue' 
                ...
                : 'bg-[#313136] text-red-400' // Счетчик: когда неактивен
            }`}>
              {overdueTasks.length}
            </span>
          </button>
          ...
          >
            ...
            <span className={`px-2.5 py-0.5 rounded-full text-[12px] font-bold ${
              activeTab === 'upcoming' 
                ...
                : 'bg-[#313136] text-[#34C759]' // Счетчик: когда неактивен
            }`}>
