...
// ==========================================
// 4. PLACES DATABASE MODAL (Redesigned)
// ==========================================
...
// ==========================================
// 4. PLACES DATABASE MODAL (FULL SCREEN iOS)
// ==========================================
interface PlacesDatabaseModalProps {
  open: boolean; onClose: () => void; places: FavoritePlace[]; setPlaces: (p: FavoritePlace[]) => void; onSelect?: (location: string) => void; t: Dictionary;
}
...
export const PlacesDatabaseModal = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-black flex flex-col" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
      ...
      {/* 1. HEADER (Как в Settings / Review) */}
      <div className="flex items-center justify-between px-4 h-11 border-b border-white/10 bg-black/80 backdrop-blur-xl sticky top-0 shrink-0">
        <button onClick={onClose} className="text-[#34C759] text-[17px] font-medium">{t.cancel || 'Close'}</button>
      ...
      {/* 2. SEARCH BAR */}
      <div className="px-4 py-3 border-b border-white/10 shrink-0 bg-[#1C1C1E]/30">
        <div className="relative">
          <Search className="w-5 h-5 absolute left-3 top-2 text-white/40" />
          <input 
            type="text" 
            value={search} 
            onChange={e => setSearch(e.target.value)} 
            placeholder={t.searchDb} 
            className="w-full bg-[#2C2C2E] border border-transparent rounded-lg pl-10 pr-4 py-2 text-[15px] text-white placeholder:text-white/40 focus:outline-none focus:border-[#34C759]/50 transition-all" 
          />
        </div>
        <p className="text-[11px] text-white/30 mt-2 ml-1 uppercase tracking-widest font-medium">{t.found}: {filteredPlaces.length}</p>
      </div>

      {/* 3. CARDS LIST */}
      <div className="flex-1 overflow-y-auto px-4 py-4 pb-[calc(env(safe-area-inset-bottom)+24px)] space-y-4 custom-scrollbar">
        {filteredPlaces.length === 0 ? (
          ...
          filteredPlaces.map((place, index) => {
            ...
            <div 
              key={place.id} 
              className={`bg-[#1C1C1E] rounded-2xl p-4 border-l-4 ${borderColorClass} transition-transform active:scale-[0.98] ${onSelect ? 'cursor-pointer hover:bg-white/5' : ''}`}
              onClick={() => { 
                ...
                <div className="flex justify-between items-start gap-4">
                  ...
                  {!onSelect && (
                    <div className="flex items-center gap-1 shrink-0" onClick={e => e.stopPropagation()}>
                      <button onClick={() => startEdit(place)} className="p-2.5 text-white/30 hover:text-white hover:bg-white/5 rounded-xl transition-all"><Edit2 className="w-4 h-4" /></button>
