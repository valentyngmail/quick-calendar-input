...
export const ReviewScreen = ({ 
  ...
  return (
    <div className="fixed inset-0 bg-[var(--bg-main)] z-[150] flex flex-col pt-safe animate-in slide-in-from-bottom-4 duration-300">
      ...
      {/* HEADER (Убрали бесполезную галочку, но оставили блок для центровки) */}
      <div className="flex items-center justify-between px-4 h-16 bg-[var(--bg-main)]/80 backdrop-blur-xl sticky top-0 shrink-0 z-30 border-b border-white/5">
        ...
        <div className="w-[80px] flex justify-end">
          {/* Пустой блок для балансировки Flexbox */}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 pt-4 custom-scrollbar transition-all duration-300" style={{ paddingBottom: `calc(150px + ${kbOffset}px)` }}>
        ...
        {/* TITLE (Сделали rows=1 и авто-высоту) */}
        <div className="pt-2 pb-6">
          <textarea 
            ...
            className="w-full bg-transparent text-white text-[32px] leading-tight font-bold outline-none resize-none placeholder:text-white/20 overflow-hidden" 
            rows={1} 
          />
        </div>
        ...
        <div className="bg-[var(--bg-surface)] rounded-[20px] border border-white/5 overflow-hidden mb-4 p-5 shadow-lg">
          ...
          <textarea 
            ...
            className="w-full bg-transparent text-[16px] leading-relaxed text-white/90 outline-none resize-none placeholder:text-white/20 overflow-hidden" 
            rows={4} 
          />
        </div>
      </div>

      {/* BOTTOM ACTION BUTTON (Исправлена форма и высота под стиль главного экрана) */}
      <div className="fixed bottom-0 left-0 right-0 pt-10 pb-safe-16 px-6 z-20 pointer-events-none" 
           style={{ background: 'linear-gradient(to top, rgba(0,0,0,1) 0%, rgba(0,0,0,0.9) 40%, rgba(0,0,0,0) 100%)' }}>
        <button 
          onClick={onSave} 
          className="w-full h-[54px] bg-[var(--primary)] text-white font-semibold text-[17px] rounded-full shadow-[0_8px_30px_rgba(var(--primary-rgb),0.3)] active:scale-97 transition-transform pointer-events-auto"
        >
