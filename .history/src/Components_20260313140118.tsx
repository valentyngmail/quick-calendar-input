...
export const PlacesDatabaseModal = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-black/60 backdrop-blur-sm flex items-end justify-center sm:items-center sm:p-4 transition-all" onClick={onClose}>
      ...
      {/* Контейнер теперь выглядит как шторка iOS (прижат вниз на мобилке) */}
      <div className="bg-[#1C1C1E] w-full max-w-lg h-[85vh] rounded-t-[32px] sm:rounded-3xl flex flex-col shadow-2xl relative" onClick={e => e.stopPropagation()}>
        ...
        {/* Список карточек (Скролл уходит прямо в пол) */}
          ...
          {filteredPlaces.length === 0 ? (
            ...
            filteredPlaces.map((place, index) => {
              ...
              <div 
                ...
                onClick={() => { 
                  // Если мы в режиме выбора, клик по карточке ВЫБИРАЕТ адрес!
                ...
                {editingId === place.id ? (
                  ...
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div 
                        onClick={(e) => {
                          // Если мы НЕ в режиме выбора — открываем карты.
                          // Если в режиме выбора — ничего не делаем, даем клику дойти до карточки.
                    ...
                    {/* Кнопки редактирования скрываем, если мы в режиме выбора, чтобы не отвлекали */}
                    {!onSelect && (
