...
const VoiceCalendarApp = () => {
  ...
  // --- Keyboard & Viewport Sync (ЖЕСТКАЯ ФИКСАЦИЯ ДЛЯ IOS) ---
  useEffect(() => {
    const sync = () => {
      ...
      // Если Safari попытался проскроллить страницу вверх - безжалостно возвращаем обратно
      if (window.scrollY > 0) {
        window.scrollTo(0, 0);
      }
    ...
    // "Серебряная пуля": намертво фиксируем body, чтобы Safari не мог его сдвинуть
    ...
    window.visualViewport?.addEventListener('scroll', sync); // Ловим скролл вьюпорта
    window.addEventListener('scroll', sync); // Ловим скролл окна
    sync(); 
  ...
  const searchLocation = useCallback(async (query: string) => {
    ...
    try {
      // Добавили addressdetails=1 и язык интерфейса, чтобы получать точные данные
  ...
  const handleSendForDecoding = async (text: string) => {
    ...
    try {
      ...
      // Логика добавления в Хвосты, если это задача
      if (newEvent.isTask) {
  ...
  const handleConfirm = async () => {
    ...
    if (settings.saveWebhook === 'test') {
      ...
      setTimeout(() => {
        ...
        // ВОТ ЧТО Я ЗАБЫЛ: Сохраняем задачу в Хвосты даже в тестовом режиме
  ...
  const handleReschedule = (task: ParsedEvent) => {
    // Копируем задачу как новое событие, переводим в фазу ревью и закрываем список
  ...
  return (
    <div className="fixed inset-0 bg-black text-white overflow-hidden flex flex-col">
      ...
      {/* 1. HEADER (Исправленная верстка) */}
      <div className="absolute top-0 left-0 right-0 px-4 flex items-center z-50 h-header pt-safe">
        ...
        <div className="flex-1 flex items-center gap-1">
          ...
          <button onClick={() => setShowTasks(true)} className="p-2 rounded-xl active:bg-white/10 transition-colors relative" title="Pending Tasks">
            {(() => {
              // Вычисляем, есть ли реально просроченные задачи (или задачи на сегодня)
              ...
              return (
                <>
                  <CheckCircle2 className={`w-5 h-5 ${overdueCount > 0 ? "text-red-500" : hasTasks ? "text-[#34C759]" : "text-white/40"}`} />
                  {overdueCount > 0 && (
        ...
        <div className="text-xl font-bold tracking-tight">
          Voice<span className="text-[#34C759]">Cal</span>
        </div>
        ...
        <div className="flex-1 flex justify-end items-center">
          <div className="relative">
            ...
            {!settingsConfigured && (
              <span className="absolute top-1.5 right-1.5 w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse border border-black" />
            )}
          </div>
        </div>
      </div>
      ...
      <div ref={mainRef} className="absolute top-[90px] left-0 right-0 bottom-0 flex flex-col transition-all duration-300">
        ...
        <div className="flex-1 flex flex-col justify-center px-8 pb-[80px] w-full max-w-md mx-auto relative">
          ...
          {phase === 'success' && <div className="flex flex-col items-center gap-4"><CheckCircle2 className="text-[#34C759]" size={60} /><p className="text-xl font-bold">{t.eventSaved}</p></div>}
      ...
      {phase === 'idle' && (
        <div ref={dockRef} className="absolute left-6 right-6 flex justify-center bottom-safe-16 z-50 transition-all duration-300">
          <div className="glass-panel bg-[#1C1C1E]/80 backdrop-blur-2xl border border-white/10 border-t-white/20 rounded-[32px] p-1.5 flex items-center justify-between w-full max-w-sm shadow-2xl">
            ...
            <button onClick={handleProcessText} className="h-11 px-8 ml-2 bg-[#34C759] text-black rounded-[22px] font-bold active:scale-95 transition-all">{t.process}</button>
          </div>
        </div>
      )}
      ...
      {showDebug && (phase === 'idle' || phase === 'error') && (
        <div className="fixed inset-0 z-[400] bg-black flex flex-col p-6 pt-16">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-[#34C759] font-bold text-xl flex items-center gap-2"><Bug /> {t.systemLogs}</h3>
          ...
          <div className="flex-1 overflow-y-auto space-y-3 mb-6 font-mono text-xs text-[#34C759] bg-[#1C1C1E] p-4 rounded-xl">
            {debugLogs.length === 0 ? <span className="opacity-50">{t.noLogs}</span> : debugLogs.map((log, i) => <p key={i} className="break-all border-b border-[#34C759]/20 pb-2">{log}</p>)}
          </div>
          <div className="flex gap-4 pb-8">
            <button onClick={() => { navigator.clipboard.writeText(debugLogs.join('\n')); toast.success(t.copied); }} className="flex-[2] py-4 bg-[#34C759] text-black rounded-xl font-bold text-lg active:scale-95 transition-transform">{t.copyLogs}</button>
            <button onClick={() => setDebugLogs([])} className="flex-1 py-4 bg-[#1C1C1E] text-white rounded-xl font-bold text-lg active:scale-95 transition-transform">{t.clearLogs}</button>
          </div>
        </div>
      )}
