...
const VoiceCalendarApp = () => {
  ...
  const [isKbOpen, setIsKbOpen] = useState(false); // Для скрытия истории
  ...
  useEffect(() => {
    const sync = () => {
      ...
      // Скрываем историю, если открыта клавиатура
      setIsKbOpen(kbHeight > 50);
    };
  ...
  const formatHistoryDate = (d: string, time: string) => {
    if (!d) return time;
    const parts = d.split('-'); // "2026-03-14"
    if (parts.length === 3) {
  ...
  return (
    <div className="fixed inset-0 bg-black text-white flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex flex-col justify-center items-center relative transition-all duration-300">
        
        {/* Горизонтальный скролл истории (как в index.html) */}
        {phase === 'idle' && history.length > 0 && (
          <div 
            className={`absolute top-0 left-0 right-0 z-10 flex gap-3 px-6 pt-2 pb-6 overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden transition-all duration-300 ${isKbOpen ? 'opacity-0 -translate-y-4 pointer-events-none' : 'opacity-100 translate-y-0'}`}
          >
            {history.map((item, i) => (
              <div 
                key={`${item.id}-${i}`} 
                onClick={(e) => { e.stopPropagation(); handleDuplicate(item); }}
                className="bg-white/10 backdrop-blur-xl border border-white/5 border-top-white/10 rounded-[20px] py-3 px-4 flex items-center gap-4 shrink-0 max-w-[240px] cursor-pointer hover:bg-white/20 transition-transform active:scale-[0.96]"
              >
        ...
        {phase === 'idle' && (
          <div className="px-8 w-full flex justify-center pb-16">
            <textarea
        ...
        {phase === 'success' && <div className="flex flex-col items-center gap-4 pb-16"><CheckCircle2 className="text-[#34C759]" size={60} /><p className="text-xl font-bold">Event Saved!</p></div>}

        {phase === 'error' && (
          <div className="bg-[#1C1C1E] p-6 rounded-2xl border border-red-500/30 text-center max-w-xs z-10 pb-16">
