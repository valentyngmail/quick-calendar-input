...
import { 
  Square, Check, X, Copy, Calendar, MapPin, Clock, Type, Timer, 
  RotateCcw, Send, Trash2, Bug, CheckCircle2, AlertTriangle, 
  Loader2, Users, Zap, FileText, Settings, BookOpen 
} from 'lucide-react';
import { toast } from 'sonner';

// ВСЁ БЕРЕМ ИЗ НАШИХ НОВЫХ ФАЙЛОВ
import { AppPhase, ParsedEvent, AppSettings, FavoritePlace, DICT, SETTINGS_KEY, FAV_PLACES_KEY, loadSettings, saveSettings } from './Core';
import { InputField, SettingsModal, PlacesDatabaseModal } from './Components';
...
const VoiceCalendarApp = () => {
  ...
  <div className="fixed inset-0 bg-black z-[150] flex flex-col" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
    ...
    <div className="flex items-center justify-between px-4 h-11 border-b border-white/10 bg-black/80 backdrop-blur-xl sticky top-0">
      <button onClick={resetToIdle} className="text-[#FF453A] text-[17px] font-medium">Cancel</button>
    ...
    <div className="flex-1 overflow-y-auto px-4 pb-32">
      ...
      <div className="bg-[#1C1C1E] rounded-xl overflow-hidden mb-6">
        <InputRow label={t.date}>
          <input type="date" value={parsedEvent.date} onChange={e => setParsedEvent({...parsedEvent, date: e.target.value})} className="bg-transparent text-right outline-none text-[#34C759] font-semibold" />
        </InputRow>
        <InputRow label={t.time}>
          <input type="time" value={parsedEvent.time} onChange={e => setParsedEvent({...parsedEvent, time: e.target.value})} className="bg-transparent text-right outline-none text-[#34C759] font-semibold" />
        </InputRow>
        <InputRow label="Duration">
          <select 
            ...
            className="bg-transparent text-white/60 outline-none appearance-none"
          >
      ...
      {/* Группа 2: Место и Гости */}
      <div className="bg-[#1C1C1E] rounded-xl overflow-hidden mb-6">
        <InputRow label={t.location}>
          <input type="text" value={parsedEvent.location} onChange={handleLocationChange} className="bg-transparent text-right outline-none text-white/60 w-full" placeholder="Add location" />
        </InputRow>
        <InputRow label="Guests">
          <input type="text" value={parsedEvent.guests} onChange={e => setParsedEvent({...parsedEvent, guests: e.target.value})} className="bg-transparent text-right outline-none text-white/60 w-full" placeholder="emails" />
        </InputRow>
      </div>
    ...
    <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black via-black/90 to-transparent pb-[calc(env(safe-area-inset-bottom)+16px)]">
      <button 
        ...
        className="w-full h-14 bg-gradient-to-br from-[#34C759] to-[#28a745] text-black font-bold rounded-2xl shadow-[0_8px_20px_rgba(52,199,89,0.2)] active:scale-95 transition-transform"
      >
        Save to Calendar
      </button>
    </div>
