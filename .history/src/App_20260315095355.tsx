...
const VoiceCalendarApp = () => {
  ...
  const handleSendForDecoding = async (text: string) => {
    ...
    try {
      // --- ПЕРЕДАЕМ ТОЧНОЕ ВРЕМЯ И ЗОНУ ДЛЯ ИИ ---
      const now = new Date();
      ...
      const res = await fetch(settings.decodeWebhook, {
        ...
        body: JSON.stringify({ 
          ...
          currentDate, 
          currentTime,  // Точное время на устройстве
          timeZone,     // Часовой пояс (например, Europe/Berlin)
      ...
      // --- ЧИТАЕМ ЗАГОЛОВКИ С ТОКЕНАМИ ИЗ MAKE.COM ---
      const tokens = res.headers.get('x-total-tokens');
      const model = res.headers.get('gemini-model') || 'Gemini';

      // 👇 ДОБАВЛЯЕМ ЛОГИРОВАНИЕ УСПЕХА В ДЕБАГ-КОНСОЛЬ 👇
      addLog(`✅ [DECODE SUCCESS] Model: ${model} | Tokens: ${tokens || 'N/A'}`);
      addLog(`📥 [DECODE PAYLOAD] ${rawText}`);
      // 👆 ============================================== 👆

      // 1. Собираем описание: Сначала ответ ИИ (только перевод), затем оригинальный текст
      let finalDescription = data.description 
        ? `${data.description}\n\n🗣️ ${text}`
        : `🗣️ ${text}`;
        
      // 2. Приклеиваем статистику токенов через две строки (\n\n)
      if (tokens) {
  ...
  const handleConfirm = async () => {
    ...
    // чтобы Make.com не падал от пустых коллекций
  ...
  return (
    <div className="fixed inset-0 bg-[var(--bg-main)] text-white overflow-hidden flex flex-col">
      ...
      {/* 1. HEADER (Идеальный баланс, без лишнего шума) */}
        ...
        <div className="flex-1 flex justify-start">
          <button onClick={() => setShowTasks(true)} className="p-2 rounded-xl active:bg-white/10 transition-colors relative" title="Pending Tasks">
            {(() => {
              ...
              // Если нет просроченных - иконка сливается с фоном (text-white/30). Никакого кричащего зеленого без повода.
              return (
                <>
        ...
        <div className="flex-1 flex flex-col justify-center px-6 pb-[100px] w-full max-w-md mx-auto relative">
          {phase === 'idle' && (
            <div className={`relative transition-all duration-500 w-full ${isFocused ? 'scale-[1.02]' : 'scale-100'}`}>
              {/* Премиальный подложечный контейнер, который исчезает при фокусе */}
              <div className={`absolute inset-0 bg-white/[0.03] border border-white/[0.05] rounded-[32px] transition-all duration-500 ${isFocused || textInput.length > 0 ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`} />
      ...
      <PlacesDatabaseModal 
        ...
        // ВАЖНО: Передаем onSelect ТОЛЬКО если мы находимся на экране проверки деталей!
