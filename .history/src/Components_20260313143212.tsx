...
export const ReviewScreen = ({ 
  ...
  <div className="fixed inset-0 bg-black z-[150] flex flex-col" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
    ...
    <div className="flex-1 overflow-y-auto px-4 pb-32">
      <div className="py-6">
        <textarea value={parsedEvent.title} onChange={e => setParsedEvent({...parsedEvent, title: e.target.value})} className="w-full bg-transparent text-white text-[24px] font-bold outline-none resize-none placeholder:text-white/20" rows={2} />
      </div>
      ...
      <div className="bg-[#1C1C1E] rounded-xl overflow-hidden mb-6">
        ...
        <InputRow label={t.duration}>
          <select value={parsedEvent.duration} onChange={e => setParsedEvent({...parsedEvent, duration: e.target.value})} className="bg-transparent text-white text-right outline-none appearance-none w-full cursor-pointer">
      ...
      <div className="bg-[#1C1C1E] rounded-xl mb-6 relative">
        <InputRow label={t.location}>
          <input type="text" value={parsedEvent.location} onChange={e => onLocationChange(e.target.value)} className="bg-transparent text-right outline-none text-white w-full placeholder:text-white/20" placeholder="Address" />
        ...
        <InputRow label={t.guestsLabel}>
          <input type="text" value={parsedEvent.guests} onChange={e => setParsedEvent({...parsedEvent, guests: e.target.value})} className="bg-transparent text-right outline-none text-white w-full placeholder:text-white/20" placeholder="email1, email2" />
        </InputRow>
      </div>

      <div className="bg-[#1C1C1E] rounded-xl overflow-hidden mb-6 p-4">
        <label className="text-[13px] text-white/40 mb-2 font-bold uppercase tracking-widest block">{t.descLabel}</label>
        <textarea value={parsedEvent.description} onChange={e => setParsedEvent({...parsedEvent, description: e.target.value})} className="w-full bg-transparent text-[15px] text-white outline-none resize-none placeholder:text-white/20" rows={4} />
      </div>
    ...
    <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black via-black/90 to-transparent pb-[calc(env(safe-area-inset-bottom)+16px)]">
      <button onClick={onSave} className="w-full h-14 bg-gradient-to-br from-[#34C759] to-[#28a745] text-black font-bold text-[17px] rounded-2xl shadow-lg active:scale-95 transition-transform">{t.saveToCalendar}</button>
    </div>
  </div>
);