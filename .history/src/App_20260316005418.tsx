...
const VoiceCalendarApp = () => {
  ...
  useEffect(() => {
    ...
    const sync = () => {
      // Идеальное отслеживание: передаем реальную высоту и смещение камеры в CSS
    ...
    // Слушаем не только изменение размера, но и попытки iOS "проскроллить" экран
  ...
  const searchLocation = useCallback(async (query: string) => {
    ...
    try {
      const res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}&limit=8&addressdetails=1`, { headers: { 'Accept-Language': `${appLang},en;q=0.9` }});
      ...
      // Добавили строгий тип : string[] вот здесь 👇
      const rawSuggestions: string[] = data.map((item: OSMItem) => {
        const addr = item.address;
        const fallback = item.display_name.split(',').map(s => s.trim()).slice(0, 3).join(', ');
        ...
        if (street && city) {
          const baseAddress = `${street}${house}, ${city}`;
          if (item.name && item.name !== street && item.name !== city) {
            return `${item.name}, ${baseAddress}`;
          }
          return baseAddress;
        }
        return fallback;
      ...
      // Теперь TypeScript абсолютно уверен, что мы работаем со строками
      const uniqueSuggestions = Array.from(new Set(rawSuggestions)).filter(s => s.length > 0).slice(0, 3);
  ...
  return (
    <div 
      className="absolute left-0 right-0 bg-[var(--bg-main)] text-white overflow-hidden flex flex-col" 
      style={{ 
    ...
    >
          ...
          {phase === 'idle' && (
            <div className={`relative transition-all duration-500 w-full ${isFocused ? 'scale-[1.02]' : 'scale-100'}`}>
              <textarea
                ...
                rows={1}
                className="w-full bg-transparent text-center text-[28px] leading-tight font-semibold outline-none resize-none py-10 px-6 relative z-10 custom-scrollbar premium-input"
              />
            </div>
          )}
      ...
      {/* 3. DOCK (Разделенные кнопки) */}
      {phase === 'idle' && (
        <div className="absolute left-6 right-6 flex justify-center z-50 transition-all duration-300" style={{ bottom: 'calc(env(safe-area-inset-bottom) + 20px)' }}>
          <div className="flex items-center gap-3 w-full max-w-sm">
            
            {/* Круглый переключатель языка (отдельная парящая кнопка) */}
            <button 
              onClick={() => {
            ...
            {/* Кнопка Erstellen (отдельная парящая кнопка) */}
            <button 
              onClick={handleProcessText} 
              disabled={!textInput.trim()}
              className={`flex-1 h-[54px] rounded-full font-semibold text-[17px] flex items-center justify-center gap-2 transition-all duration-300 shadow-xl ${
                textInput.trim() 
                  ? 'bg-[var(--primary)] text-white border border-[var(--primary)] shadow-[0_0_20px_rgba(var(--primary-rgb),0.3)] active:scale-[0.97]' 
                  : 'bg-[#1C1C1E]/80 backdrop-blur-2xl border border-white/10 text-white/30 cursor-default'
              }`}
            >
              {t.process || "Erstellen"}
              <ArrowUp 
                size={20} 
                strokeWidth={2.5} 
                className={`transition-colors duration-300 ${textInput.trim() ? 'text-white/90' : 'text-white/20'}`} 
              />
            </button>
          </div>
        </div>
