...
export const SettingsModal = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-[var(--bg-main)] flex flex-col overflow-hidden pt-safe">
      ...
      {/* HEADER: Сохранение теперь здесь, как в iOS */}
      ...
      >
        ...
        {/* WEBHOOKS: Теперь в стиле iOS-ячеек */}
        ...
        {/* КНОПКИ ДЕЙСТВИЙ: Центрированные, без лишних иконок */}
    ...
    return (
      <div className="px-4 py-3 border-b border-white/10 bg-[var(--bg-surface)] animate-in fade-in duration-200">
        ...
        <div className="relative">
          ...
          />
          {showDropdown && (
            <div className="absolute z-50 left-0 right-0 top-full mt-1 bg-[var(--bg-surface-elevated)] border border-white/10 rounded-xl shadow-2xl max-h-48 overflow-y-auto custom-scrollbar mb-3">
        ...
        <div className="flex justify-end gap-4">
          <button onClick={() => { cancelEdit(); setShowDropdown(false); }} className="text-white/40 font-medium text-[15px]">{t.cancel}</button>
          <button onClick={() => { saveEdit(place.id); setShowDropdown(false); }} className="text-[var(--primary)] font-bold text-[15px]">{t.saveBtn}</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-[var(--bg-main)] flex flex-col pt-safe">
      ...
      <div className="flex-1 overflow-y-auto pb-safe-24 custom-scrollbar mask-linear-gradient relative">
        ...
        {isAdding && (
          <div className="px-4 py-4 border-b border-white/10 bg-[var(--bg-surface-elevated)] animate-in fade-in slide-in-from-top-2 duration-200 relative">
            <input 
              value={addForm.title} 
            ...
            <div className="relative">
              <input 
                ...
                onChange={e => {
                  ...
                  addSearchTimeoutRef.current = setTimeout(() => fetchAddSuggestions(e.target.value), 400);
                }} 
                placeholder={t.address || "Адрес"} 
              ...
              />
              {showAddDropdown && (
                <div className="absolute z-50 left-0 right-0 top-full mt-1 bg-[var(--bg-surface)] border border-white/10 rounded-xl shadow-2xl max-h-48 overflow-y-auto custom-scrollbar mb-3">
                  {addSuggestions.map((loc, i) => (
                    <button key={i} onClick={() => { setAddForm({...addForm, location: loc}); setShowAddDropdown(false); }} className="w-full text-left px-4 py-3 text-sm text-white hover:bg-white/10 border-b border-white/5 last:border-0 truncate transition-colors">
                      <MapPin size={14} className="inline mr-2 text-[var(--primary)]" /> {loc}
                    </button>
                  ))}
                </div>
            ...
            <div className="flex justify-end gap-4">
              <button onClick={() => { setIsAdding(false); setShowAddDropdown(false); }} className="text-white/40 font-medium text-[15px]">{t.cancel}</button>
              <button 
                onClick={() => { handleAddNew(); setShowAddDropdown(false); }} 
...
export const ReviewScreen = ({ 
  ...
  return (
    <div className="fixed inset-0 bg-[var(--bg-main)] z-[150] flex flex-col pt-safe">
      ...
      >
        {/* Уменьшили отступ сверху (pt-4) и сильно срезали снизу (pb-3) */}
