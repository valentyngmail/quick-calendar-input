...
const VoiceCalendarApp = () => {
  ...
  <div className="flex-1 flex flex-col px-6 pt-4">
    ...
    <div className="mb-8 flex items-center gap-3">
      ...
      <div>
        <h2 className="text-[22px] font-bold tracking-tight">{t.checkDetails}</h2>
        <p className="text-white/40 text-[13px] font-medium uppercase tracking-widest">Review & Adjust</p>
      </div>
    </div>
    ...
    <div className="space-y-6 flex-1 overflow-y-auto pr-1 custom-scrollbar">
      <InputField 
        ...
        onChange={(v) => setParsedEvent({...parsedEvent, title: v})} 
      />
      ...
      <div className="grid grid-cols-2 gap-4">
        <InputField 
          ...
          onChange={(v) => setParsedEvent({...parsedEvent, date: v})} 
        />
        <InputField 
          ...
          onChange={(v) => setParsedEvent({...parsedEvent, time: v})} 
        />
      </div>
      ...
      <div className="relative">
        ...
        />
        {/* Кнопка открытия базы мест */}
        <button 
          onClick={() => setShowDatabase(true)}
          className="absolute right-3 bottom-3 p-1.5 bg-white/5 hover:bg-white/10 rounded-md transition-colors"
        >
    ...
    <div className="py-8 flex gap-3">
      <button 
        ...
        className="flex-[2] h-14 bg-[#34C759] text-black font-bold rounded-[22px] flex items-center justify-center gap-2 active:scale-95 transition-transform"
      >
        <Check className="w-5 h-5" /> 
        <span className="text-[17px]">{t.confirm}</span>
      </button>
      <button 
        onClick={resetToIdle} 
        className="flex-1 h-14 bg-white/10 text-white font-bold rounded-[22px] flex items-center justify-center gap-2 active:scale-95 transition-transform"
      >
        <X className="w-5 h-5" />
        <span className="text-[17px]">{t.cancel}</span>
      </button>
    </div>
  </div>
)}
