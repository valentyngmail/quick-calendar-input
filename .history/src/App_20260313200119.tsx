  ...
  return (
    <div className="fixed inset-0 bg-[var(--bg-main)] text-white overflow-hidden flex flex-col">
      ...
      <div className="absolute top-0 left-0 right-0 px-4 flex items-center z-50 h-header pt-safe">
        ...
        <div className="flex-1 flex items-center gap-1">
          ...
          <button onClick={() => setShowTasks(true)} className="p-2 rounded-xl active:bg-white/10 transition-colors relative" title="Pending Tasks">
            {(() => {
              ...
              return (
                <>
                  {/* Иконка использует CSS переменные Успеха и Тревоги */}
                  <CheckCircle2 className={`w-5 h-5 ${overdueCount > 0 ? "text-[var(--danger)]" : hasTasks ? "text-[var(--success)]" : "text-white/40"}`} />
                  {overdueCount > 0 && (
                    <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-[var(--danger)] rounded-full border border-black animate-pulse" />
                  )}
                </>
              );
            })()}
        ...
        {/* Центр: Логотип (Использует Primary переменную) */}
        ...
        <div className="flex-1 flex justify-end items-center">
          <div className="relative">
            ...
            {!settingsConfigured && (
              <span className="absolute top-1.5 right-1.5 w-2.5 h-2.5 bg-[var(--danger)] rounded-full animate-pulse border border-black" />
            )}
          </div>
        </div>
      </div>
