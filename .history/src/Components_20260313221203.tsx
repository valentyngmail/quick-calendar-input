...
// ==========================================
// 4. PLACES DATABASE MODAL (Идеальный Swipe и кастомный Undo)
// ==========================================
...
const SwipeablePlaceItem = ({ 
  ...
  return (
    <div className="relative border-b border-white/5 overflow-hidden bg-[var(--bg-main)]">
      {/* ИСПРАВЛЕНО: Теперь карточка и кнопки в одном flex-контейнере, который сдвигается целиком */}
      <div 
...
export const PlacesDatabaseModal = ({ open, onClose, places, setPlaces, onSelect, t }: PlacesDatabaseModalProps) => {
  ...
  const handleDelete = (id: string) => {
    ...
    // Показываем кастомный Undo-снэкбар
    setLastDeleted(placeToDelete);
    if (undoTimeoutRef.current) clearTimeout(undoTimeoutRef.current);
    undoTimeoutRef.current = setTimeout(() => setLastDeleted(null), 4000);
  };
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-[var(--bg-main)] flex flex-col pt-safe relative">
      ...
      <div className="flex-1 overflow-y-auto pb-safe-24 custom-scrollbar mask-linear-gradient">
      ...
      {/* КАСТОМНЫЙ UNDO СНЭКБАР */}
      {lastDeleted && (
        <div className="absolute bottom-[calc(env(safe-area-inset-bottom)+32px)] left-4 right-4 mx-auto max-w-sm bg-[#2C2C2E]/95 backdrop-blur-xl border border-white/10 rounded-[14px] p-4 flex items-center justify-between shadow-2xl z-[400] animate-in slide-in-from-bottom-5 fade-in duration-300">
