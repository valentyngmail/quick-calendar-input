    ...
    <div className="fixed inset-0 bg-[var(--bg-main)] text-white overflow-hidden flex flex-col">
      ...
      {showDebug && (phase === 'idle' || phase === 'error') && (
        <div className="fixed inset-0 z-[500] bg-[var(--bg-main)] flex flex-col pt-safe animate-in fade-in duration-200">
          ...
          {/* Консоль логов в стиле Inset Grouped */}
          <div className="flex-1 overflow-y-auto px-4 pt-4 pb-safe-24 custom-scrollbar">
            <div className="bg-[var(--bg-surface)] rounded-xl p-4 min-h-[50vh] border border-white/5 shadow-sm font-mono text-[13px] leading-relaxed text-[var(--primary)] overflow-hidden">
              {debugLogs.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-40 opacity-30 text-white">
                  <p>{t.noLogs || 'No logs'}</p>
                </div>
              ) : (
                debugLogs.map((log, i) => (
                  <p key={i} className="break-all border-b border-white/5 last:border-0 py-2.5 first:pt-0 last:pb-0">{log}</p>
                ))
              )}
            </div>
          </div>
