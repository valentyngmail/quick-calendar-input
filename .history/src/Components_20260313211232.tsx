...
// --- Подкомпонент: Элемент списка со свайпом ---
const SwipeablePlaceItem = ({ 
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    // Если скроллим вверх-вниз — отменяем горизонтальный свайп
    ...
    // Тянем влево (до -190px для трех кнопок)
  ...
  return (
    <div className="relative border-b border-white/5 overflow-hidden bg-[var(--bg-main)]">
      ...
      <div 
        ...
        className="px-4 py-3 bg-[var(--bg-main)] relative z-10 w-full flex items-center justify-between active:bg-white/5 transition-colors"
        style={{ 
...
export const PlacesDatabaseModal = ({ open, onClose, places, setPlaces, onSelect, t }: PlacesDatabaseModalProps) => {
  ...
  const handleDelete = (id: string) => {
    // Делаем мягкое удаление (без страшного window.confirm) с возможностью отмены
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-[var(--bg-main)] flex flex-col pt-safe">
      ...
      {/* SEARCH BAR (Липкий) */}
      <div className="px-4 py-3 border-b border-white/10 shrink-0 bg-[var(--bg-surface)]/80 backdrop-blur-md">
