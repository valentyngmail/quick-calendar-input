...
// ==========================================
// 4. PLACES DATABASE MODAL (ИДЕАЛЬНАЯ ФИЗИКА iOS)
// ==========================================
...
const SwipeablePlaceItem = ({ 
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    ...
    // ВАЖНО: Мы НЕ ставим isDragging=true здесь, чтобы не убить нативный скролл Safari
  };
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    // Определяем, что делает юзер: скроллит или свайпает?
    if (!isDragging) {
      if (Math.abs(deltaY) > 5) {
        // Юзер потянул вниз/вверх. Позволяем браузеру скроллить.
        isScrolling.current = true;
        return;
      }
      if (Math.abs(deltaX) > 5) {
        // Юзер потянул вбок. Только теперь включаем режим свайпа.
        setIsDragging(true);
      }
    } else {
      // Идет свайп. Двигаем карточку.
      let newX = startOffsetX.current + deltaX;
      if (newX > 0) newX = 0;
      if (newX < -190) newX = -190;
      setOffsetX(newX);
    }
  };
  ...
  return (
    <div className="relative border-b border-white/5 overflow-hidden bg-[var(--bg-main)]">
      <div 
...
export const PlacesDatabaseModal = ({ open, onClose, places, setPlaces, onSelect, t }: PlacesDatabaseModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-[var(--bg-main)] flex flex-col pt-safe relative">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar mask-linear-gradient relative">
      ...
      {/* ИСПРАВЛЕННЫЙ СНЭКБАР: Идеальное расположение на экране без конфликта с Safe Area */}
      {lastDeleted && (
        <div 
          className="absolute left-4 right-4 mx-auto max-w-sm bg-[#2C2C2E]/95 backdrop-blur-xl border border-white/10 rounded-[14px] p-4 flex items-center justify-between shadow-2xl z-[500] animate-in slide-in-from-bottom-5 fade-in duration-300"
          style={{ bottom: 'calc(env(safe-area-inset-bottom) + 24px)' }}
        >
          <span className="text-white text-[15px] font-medium">{t.deleted}</span>
          <button onClick={handleUndo} className="text-[var(--info)] font-bold text-[15px] active:opacity-50 transition-opacity px-3 py-1.5 bg-[var(--info)]/10 rounded-lg">
            {t.cancel || 'Undo'}
          </button>
        </div>
      )}
