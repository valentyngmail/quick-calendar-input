...
export const PlacesDatabaseModal = ({ 
  ...
  // Массив цветов для случайной обводки (как в референсе, где был зеленый и синий)
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
          {filteredPlaces.length === 0 ? (
            ...
            filteredPlaces.map((place, index) => {
              // Назначаем цвет на основе индекса, чтобы они шли по кругу
              ...
              <div 
                ...
                onClick={() => { if(onSelect && editingId !== place.id) onSelect(place.location); }}
              >
                {editingId === place.id ? (
                  ...
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div 
                        ...
                        className={`flex items-start gap-1.5 text-white/40 hover:text-[#34C759] transition-colors ${!onSelect ? 'cursor-pointer' : ''}`} 
                    ...
                    <div className="flex items-center gap-1 shrink-0" onClick={e => e.stopPropagation()}>
                      <button onClick={() => startEdit(place)} className="p-2 text-white/30 hover:text-white rounded-lg transition-colors"><Edit2 className="w-4 h-4" /></button>
                      <button onClick={() => handleDelete(place.id)} className="p-2 text-white/30 hover:text-[#FF453A] rounded-lg transition-colors"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </div>
                )}
              </div>
