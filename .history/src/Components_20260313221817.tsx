...
const SwipeablePlaceItem = ({ 
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    // Отменяем свайп, если юзер скроллит вниз/вверх
  ...
  return (
    <div className="relative border-b border-white/5 overflow-hidden bg-[var(--bg-main)]">
      ...
      {/* ИСПРАВЛЕНА ФИЗИКА И СКРОЛЛ: touch-action: pan-y позволяет браузеру скроллить список! */}
      <div 
        ...
        style={{ 
          ...
          touchAction: 'pan-y' // <--- ЭТОТ ПАРАМЕТР ЧИНИТ СКРОЛЛ
        }}
      >
        {/* Сама карточка (сдвигается вместе с кнопками) */}
        <div 
          onClick={() => {
        ...
        {/* Кнопки прицеплены справа к карточке */}
        {!onSelect && (
...
export const PlacesDatabaseModal = ({ open, onClose, places, setPlaces, onSelect, t }: PlacesDatabaseModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-[var(--bg-main)] flex flex-col pt-safe">
      ...
      {/* ИСПРАВЛЕНА ПЛАШКА UNDO: Теперь использует валидные Tailwind-классы (bottom-12) */}
      {lastDeleted && (
        <div className="absolute bottom-12 left-4 right-4 mx-auto max-w-sm bg-[#2C2C2E]/95 backdrop-blur-xl border border-white/10 rounded-[14px] p-4 flex items-center justify-between shadow-2xl z-[500] animate-in slide-in-from-bottom-5 fade-in duration-300">
