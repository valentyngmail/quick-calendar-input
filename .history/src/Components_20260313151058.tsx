    ...
    <div className="flex-1 overflow-y-auto px-4 pb-32">
      ...
      <div className="bg-[#1C1C1E] rounded-xl overflow-hidden mb-6">
        ...
        <InputRow label={t.duration}>
          {/* Добавили w-full, чтобы элемент растянулся и text-right сработал корректно */}
          <select 
            value={parsedEvent.duration} 
            onChange={e => setParsedEvent({...parsedEvent, duration: e.target.value})} 
            className="bg-transparent text-[#34C759] font-bold text-right outline-none appearance-none cursor-pointer w-full px-0"
          >
