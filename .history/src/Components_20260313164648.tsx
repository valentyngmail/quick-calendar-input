    ...
    <div className="fixed inset-0 z-[300] bg-black flex flex-col" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
      ...
      <div className="flex p-4 gap-2 shrink-0">
        <button 
          ...
          className={`flex-1 py-2 rounded-xl text-[14px] font-bold transition-colors ${activeTab === 'overdue' ? 'bg-red-500/20 text-red-500 border border-red-500/30' : 'bg-[#1C1C1E] text-white/40'}`}
        >
          Хвосты ({overdueTasks.length})
        </button>
        <button 
          onClick={() => { setActiveTab('upcoming'); setExpandedId(null); }}
          className={`flex-1 py-2 rounded-xl text-[14px] font-bold transition-colors ${activeTab === 'upcoming' ? 'bg-[#34C759]/20 text-[#34C759] border border-[#34C759]/30' : 'bg-[#1C1C1E] text-white/40'}`}
        >
